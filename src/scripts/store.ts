...
export function removeFromReadingList(isbn: string) {
	const filteredReadingList = store.readingList.filter((book) => book.ISBN !== isbn);
...
createEffect(() => {
	...
	const filterbooks = initialBooks.filter(
		(book) => book.pages <= pages && (genre === "todos" || book.genre === genre) && !readingListIsbn.includes(book.ISBN)
	);
...
export function IncreaseIdxReadingList(idx: number) {
	setStore("readingList", produce((readingList) => {
